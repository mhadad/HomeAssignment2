...
//    val mainDispatcherRule = MainDispatcherRule()
  ...
  },
  {
    ...
    private var expectedCountries : List<Country_Model?> = emptyList()
    @Before
    fun init(){
        retrofitMock = MockWebServer()
        apiService = Retrofit.Builder().baseUrl(retrofitMock.url("https://gist.githubusercontent.com")).addConverterFactory(
            GsonConverterFactory.create()).build().create(API_Service::class.java)
        countriesUiModelMapper = mockk()
        viewModel = CountriesListViewModel("https://gist.githubusercontent.com")
        retrofitMock.enqueue(MockResponse().setBody(mockResponse).setResponseCode(200))
        expectedCountries  = Gson().fromJson(
            mockResponse,
            object : TypeToken<List<Country_Model?>>() {}.type
        )
    }
